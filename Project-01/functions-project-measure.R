...
outliers.function.hand_length <- function(data){
  
  Q1 <- quantile(data$hand.length, .25)
  Q2 <- quantile(data$hand.length, .75)
  IQR <- IQR(data$hand.length)
  
  # now that we know the outliers, lets subset them 
  deleted.outliers <- subset(data, data$hand.length > (Q1 - 1.5*IQR) & data$hand.length < (Q2 + 1.5*IQR))
}
...
boxplotFunction1 <- function(n1, n2, n3, n4, n5, n6){
  # box plot function with labels and colors
  boxplot(n1, n2, n3, n4, n5, n6, ylim = c(0, 300), names = c("Height", "Arm Span", "Hand Length", "Hand Width", "Arm Reach", "Age"), col = c("red","yellow","blue", "green", "purple", "orange"))
}

boxplotFunction2 <- function(n1, n2, label){
  
  # box plot function with labels and colors
  boxplot(n1, n2, xlab = label, col = c("yellow", "blue"))
}

summaryDF <- function(n1, n2){
  # summary of boxplots
  x1 <- as.data.frame.list(summary(n1))
  x2 <- as.data.frame.list(summary(n2))
  
  # combine to a df
  height.df <- rbind(x1, x2)
  
  # add column with names 
  height.df$Dataset <- c("Student", "NBA")
  height.df[, c(7, 1:6)]
}
